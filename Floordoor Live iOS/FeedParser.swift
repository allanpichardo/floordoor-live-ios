...
class FeedParser: NSObject, XMLParserDelegate {
    ...
    func parseEndOfRSS2Element(_ elementName: String, qualifiedName qName: String!) {
        ...
        else if self.currentPath == "/rss/channel/item/content:encoded" {
            self.currentFeedItem?.feedContent = self.currentElementContent
        }
        ...
        else if self.currentPath == "/rss/channel/item/featuredImage" {
            self.currentFeedItem?.featuredImage = self.currentElementContent
        }
